      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-white/10">
        ...
        <div className="flex flex-wrap items-center gap-4 bg-black/60 border border-white/10 p-3.5 rounded-2xl">
          ...
          <div className="flex items-center gap-3 pr-4 border-r border-white/10">
            <div className={`w-8 h-8 rounded-full border-2 border-amber-400/60 flex items-center justify-center ${isPlaying || isRecording ? 'animate-spin' : ''}`} style={{ animationDuration: tapeSpeed === 30 ? '1.5s' : '3s' }}>
            ...
            <div className="text-[11px] font-mono">
              ...
              <span className="text-amber-400 font-bold">{tapeSpeed} IPS</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-mono transition ${tapeSpeed === 15 ? 'bg-amber-400 text-black font-bold' : 'bg-zinc-900 text-zinc-400 hover:text-white'}`}
            >
              15 IPS
            </button>
            <button
              onClick={() => setTapeSpeed(30)}
              className={`px-2.5 py-1 rounded-lg text-xs font-mono transition ${tapeSpeed === 30 ? 'bg-amber-400 text-black font-bold' : 'bg-zinc-900 text-zinc-400 hover:text-white'}`}
            >
              30 IPS
            </button>
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-zinc-900/60 border border-white/5 p-4 rounded-2xl">
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl flex items-center gap-2 text-xs font-semibold tracking-wider uppercase transition ${isPlaying && !isRecording ? 'bg-amber-400 text-black' : 'bg-zinc-800 text-zinc-300 hover:text-white'}`}
          >
        ...
        <div className="flex items-center gap-4 text-xs font-mono">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-black font-semibold text-xs uppercase tracking-wider hover:brightness-110 transition shadow-md flex items-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {channels.map((ch) => {
          ...
          return (
            <div
              key={ch.id}
              className={`p-4 rounded-2xl border transition-all flex flex-col justify-between space-y-4 ${ch.isArmed ? 'bg-zinc-900/90 border-amber-400/40' : 'bg-zinc-900/40 border-white/5'}`}
            >
              {/* Channel Header */}
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-amber-400 font-bold uppercase">{ch.id.toUpperCase()}</span>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-[10px] font-mono text-zinc-400">
                  <span>GAIN</span>
                  <span className="text-amber-400 font-bold">{ch.gain}%</span>
                </div>
                <input
                  type="range"
                  min="0"
              ...
              <div className="grid grid-cols-3 gap-1 pt-2 border-t border-white/5">
                ...
                <button
                  ...
                  className={`py-1 rounded text-[10px] font-bold font-mono uppercase transition ${ch.isSolo ? 'bg-amber-400 text-black' : 'bg-zinc-800 text-zinc-400 hover:text-white'}`}
                >
                  SOLO
                </button>
                <button
